...
The unfilled `approval-manifest.template.json` is an operator-ready evidence packet template and must not be submitted as approval. Set `MINITOK_COMMERCIAL_APPROVAL_MANIFEST` or use `--manifest` to supply a separately reviewed manifest. The packet records the current package version plus CLI and VSIX SHA-256 values, legal/privacy approvers, support contact/SLA/escalation ownership, production deployment/database/rollback/monitoring/signer evidence, npm publication authorization, and VSIX Marketplace publisher authorization. Every requirement needs `status`, `owner`, `decision`, typed `evidenceRef`, and a non-future ISO `approvedAt`; local evidence references resolve relative to the manifest, while external references must be explicitly typed. Placeholders, missing fields, stale versions, and non-SHA-256 artifact values are rejected. Only the literal `APPROVED` status with non-empty fields can produce `PASS`. Missing, malformed, pending, or otherwise unapproved input remains `BLOCKED` or `UNVERIFIED`. Publication authorization and registry publication verification are separate approvals; authorization does not prove publication. Public read-only findings may be cited as external evidence, but they do not constitute approval or publication.
