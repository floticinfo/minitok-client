...
function planChange(file, action) { const data = readConfig(file); const container = serverContainer(data); const servers = validateServers(container.value); const before = JSON.stringify(data); if (action === "connect") servers.minitok = { command: process.execPath, args: [path.resolve(__dirname, "../../runtime/stdio-entry.js")], env: { MINITOK_MCP_AUTH_TOKEN_FILE: path.join(os.homedir(), ".minitok", "entitlement", "installation-token.json") }, disabled: false }; else delete servers.minitok; data[container.key] = servers; return { file, data, changed: before !== JSON.stringify(data), schema: container.key, backup: `${file}.bak` }; }
function register(program) { const mcp = program.command("mcp"); mcp.command("status").option("--json").action(opts => { const rows = detect(); if (opts.json) console.log(JSON.stringify(rows)); else rows.forEach(row => console.log(`${row.name}: ${row.detected ? "detected" : "not found"}`)); }); for (const action of ["connect", "disconnect"]) { mcp.command(`${action} <host>`).option("--dry-run", "preview without writing").option("--preview", "preview without writing").option("--force", "write despite an unchanged configuration").option("--no-backup", "disable backup").option("--rollback", "restore backup on failure").action((host, opts) => { const file = configs()[host]; if (!file) throw new Error(`Unsupported MCP host: ${host}`); if (action === "disconnect" && !fs.existsSync(file)) return; const plan = planChange(file, action); if (!plan.changed && !opts.force) { console.log(`${action === "connect" ? "Already connected" : "Already disconnected"} ${host}`); return; } if (opts.dryRun || opts.preview) { console.log(JSON.stringify({ action, host, file, schema: plan.schema, changed: plan.changed, backup: opts.backup !== false ? plan.backup : null })); return; } writeConfig(file, plan.data, { backup: opts.backup !== false, rollback: opts.rollback === true }); console.log(`${action === "connect" ? "Connected" : "Disconnected"} minitok ${action === "connect" ? "to" : "from"} ${host}`); }); } }
