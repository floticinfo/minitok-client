...
async function cmdAuthCustomerLogin(server, email, password) {
  if (!email || !password) {
    console.error("Usage: minitok auth customer-login <email>");
    return 1;
  }
  const result = await customerAuthRequest("/v1/auth/login", { email, password }, server);
  if (!result.ok || !result.body?.token) {
    console.error(`[error] Customer login failed: ${result.body?.error || "request failed"}`);
    return 1;
  }
...
function customerAuthRequest(endpoint, body, server) {
  ...
  return postJson(base + endpoint, body, 30000);
}
...
function register(program) {
  ...
  authCmd
    ...
    .action(async (email, options) => {
      const value = await prompt("Customer password: ");
